...
if ($key == $pkey) {
    ...
    decryptFile('serverupload/' . $pname, 'serverdownload/' . $pname.'_decrypted'   , '*&@zxor)#^!+=]' . $pkey . ')#^!+=]*&@zxor');
    //echo ("success");
    echo "<a href= 'serverdownload/decrypted_" . $pname . "' id='link' download>Download</a>";
} else {
    echo ("Wrong Key Entered!");
}
